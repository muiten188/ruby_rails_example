...
class SWF
	def self.dimensions(file)
      ...
      body = Zlib::Inflate.inflate(contents[8..length].force_encoding(Encoding::ASCII_8BIT))

			# And reconstruct the file contents to the first 8 bytes (header)
			# Plus our decompressed body
      contents = contents[0..7].force_encoding(Encoding::ASCII_8BIT) + body
		end

		# Determine the nbits of our dimensions rectangle
		nbits = contents[8].unpack('B*')[0].to_i(2) >> 3
		...
		rect = contents[8..(8 + rectbytes)].unpack(('B8' * rectbytes).to_s).join
